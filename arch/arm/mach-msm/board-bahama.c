...
static int bt_set_gpio(int on)
{
	...
	if (on) {
		rc = gpio_direction_output(BT_SYS_RST, 1);
		msleep(100);
	} else {
		if (!marimba_get_fm_status(&config) &&
				!marimba_get_bt_status(&config)) {
			gpio_set_value_cansleep(BT_SYS_RST, 0);
			rc = gpio_direction_input(BT_SYS_RST);
			msleep(100);
		}
	}
	if (rc)
...
static int config_i2s(int mode)
{
	...
	if (mode == FM_I2S_ON) {
		...
		pr_info("[FM-I2S]: %s mode = FM_I2S_ON", __func__);
		...
		pr_info("[FM-I2S]: %s mode = FM_I2S_OFF", __func__);
...
static int config_pcm(int mode)
{
	...
	if (mode == BT_PCM_ON) {
		...
		pr_info("%s mode =BT_PCM_ON", __func__);
		rc = switch_pcm_i2s_reg_mode(1);
		if (rc) {
			pr_err("switch mode failed");
			return rc;
		...
		pr_info("%s mode =BT_PCM_OFF", __func__);
		rc = switch_pcm_i2s_reg_mode(0);
		if (rc) {
			pr_err("switch mode failed");
			return rc;
...
static unsigned int msm_bahama_setup_power(void)
{
	...
	rc = gpio_request(BT_SYS_RST, "bahama sys_rst_n");
	if (rc < 0) {
