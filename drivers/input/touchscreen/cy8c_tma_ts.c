...
static ssize_t cy8c_set_event_htc(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	...
	ts_data->event_htc_enable = buglevel;

	pr_info("[ts]event htc enable = %d\n", ts_data->event_htc_enable);
	return count;
}

static ssize_t cy8c_show_event_htc(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct cy8c_ts_data *ts_data;
	ts_data = private_ts;
	return sprintf(buf, "event htc enable = %d\n", ts_data->event_htc_enable);
}
static DEVICE_ATTR(event_htc, (S_IWUSR|S_IRUGO), cy8c_show_event_htc, cy8c_set_event_htc);
...
static void cy8c_ts_work_func(struct work_struct *work)
{
	...
	if ((buf[2] & 0x0F) >= 1 && !(buf[2] & 0x10)) {
		...
		for (loop_i = 0; (loop_i < ts->finger_count) && (loop_i < 4); loop_i++) {
			...
			loop_i % 2 == 1 ? (buf[base - 1]&0x0F) - 1 : (buf[base + 5] >> 4) - 1);
			 ...
			 const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "application verion = %X, x_channel = %X, y_channel = %X\n",
	...
	printk(KERN_INFO "Start touchscreen %s in %s mode\n",
