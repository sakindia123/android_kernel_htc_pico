				...
				enum tavarua_evt_t event)
{
	...
	printk(KERN_INFO "updating event_q with event %x\n", event);
...
static void tavarua_handle_interrupts(struct tavarua_device *radio)
{
	...
	printk(KERN_INFO "INTSTAT1 <%x>\n", radio->registers[STATUS_REG1]);
	printk(KERN_INFO "INTSTAT2 <%x>\n", radio->registers[STATUS_REG2]);
	printk(KERN_INFO "INTSTAT3 <%x>\n", radio->registers[STATUS_REG3]);
...
static void fm_shutdown(struct work_struct *work)
{
	struct tavarua_device *radio = container_of(work,
					struct tavarua_device, work.work);
	printk(KERN_INFO "%s: Releasing the FM I2S GPIO\n", __func__);
	if (radio->pdata->config_i2s_gpio != NULL)
		radio->pdata->config_i2s_gpio(FM_I2S_OFF);
	printk(KERN_INFO "%s: Shutting down FM SOC\n", __func__);
...
static int tavarua_search(struct tavarua_device *radio, int on, int dir)
{
	...
	printk(KERN_INFO "%s: Search Started\n", __func__);
...
static int tavarua_fops_open(struct file *file)
{
	...
	printk(KERN_INFO "%s: initializing SoC\n", __func__);
...
static int tavarua_fops_release(struct file *file)
{
	...
	printk(KERN_INFO "%s: Disabling the IRQs\n", __func__);
	/* disable irq */
...
exit:
	printk(KERN_INFO "%s: Calling fm_shutdown\n", __func__);
	queue_delayed_work(radio->wqueue, &radio->work,
		...
		struct v4l2_control *ctrl)
{
	...
	/* start/stop search */
	case V4L2_CID_PRIVATE_TAVARUA_SRCHON:
		printk(KERN_INFO "%s: starting search\n", __func__);
	...
	case V4L2_CID_PRIVATE_TAVARUA_STATE:
		...
		if (((ctrl->value == FM_RECV) || (ctrl->value == FM_TRANS))
			...
			radio->xfr_in_progress = 0;
			radio->xfr_bytes_left = 0;
			printk(KERN_INFO "%s: turning on start..\n", __func__);
			...
			printk(KERN_INFO "%s: turning on end..\n", __func__);
		}
		/* check if off */
		else if ((ctrl->value == FM_OFF) && radio->registers[RDCTRL]) {
			printk(KERN_INFO "%s: turning off start...\n", __func__);
			...
			printk(KERN_INFO "%s: turning off end...\n", __func__);
		}
		break;
					...
					struct v4l2_frequency *freq)
{
	...
	printk(KERN_INFO "%s: Calling tavarua_set_freq\n", __func__);
				...
				struct v4l2_buffer *buffer)
{
	...
	len = buffer->length;
	printk(KERN_INFO "Address of buf_fifo: %x\n",(unsigned int)&buf_fifo[0]);
	printk(KERN_INFO "Address of buf : %x\n",(unsigned int)buf);
	printk(KERN_INFO "Address of buffer : %x\n",(unsigned int)buffer);
	printk(KERN_INFO "%s: requesting buffer %d\n", __func__, buf_type);

	if ((buf_type < TAVARUA_BUF_MAX) && (buf_type >= 0)) {
		data_fifo = &radio->data_buf[buf_type];
		printk(KERN_INFO "Address of data_fifo : %x\n",(unsigned int)data_fifo);
		if (buf_type == TAVARUA_BUF_EVENTS) {
			printk(KERN_INFO "%s: wait_event_interruptible +++\n", __func__);
	...
	if (len <= 128){
		printk(KERN_INFO "%s: kfifo_out_locked +++\n", __func__);
		buffer->bytesused = kfifo_out_locked(data_fifo, &buf_fifo[0], len,
