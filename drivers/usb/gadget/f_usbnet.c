...
#define USBNET_FUNCTION_NAME "usbnet"
...
static struct usb_string usbnet_string_defs[] = {
       [STRING_INTERFACE].s = "HTC Test Command",
       {  /* ZEROES END LIST */ },
};
...
static void usb_ether_tx_timeout(struct net_device *dev)
{
	struct usbnet_context *context = netdev_priv(dev);
	USBNETDBG(context, "%s\n", __func__);
}
...
static const struct net_device_ops usb_netdev_ops = {
	.ndo_open		= usb_ether_open,
	.ndo_stop		= usb_ether_stop,
	.ndo_start_xmit	= usb_ether_xmit,
	.ndo_validate_addr	= eth_validate_addr,
	.ndo_tx_timeout	= usb_ether_tx_timeout,
...
{
	struct usbnet_device *dev = _usbnet_dev;
	struct usbnet_context *context = dev->net_ctxt;
	int rc = -EOPNOTSUPP;
	...
	USBNETDBG(context, "usbnet_ctrlrequest "
...
static int usbnet_bind_config(struct usb_configuration *c)
{
	struct usbnet_device *dev = _usbnet_dev;
	struct usbnet_context *context;
	...
	net_dev->netdev_ops = &usb_netdev_ops;
	...
	dev->cdev = c->cdev;
	dev->function.name = USBNET_FUNCTION_NAME;
